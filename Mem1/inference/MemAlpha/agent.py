...
class MemoryAgent:
    ...
    def __init__(self, agent_config: dict = None, save_process: bool = False, out_dir: str = None, is_template: bool = False) -> None:
        ...
        if self.is_qwen and not self.is_template:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained("/hpc_stor03/sjtu_home/zijian.wang/MEM1/.cache/Memalpha-4B", **tokenizer_kwargs)
...
Please only return the query, no other text."""
    ...
    def _complete_qwen(self, messages, functions):
        ...
        if self.use_vllm:
            if self.enable_thinking:
                # 1) First generation: limited by thinking budget
                first_response = self._server_completion(
                    prompt=text,
                ...
                first_ids = self.tokenizer(first_response, add_special_tokens=False).input_ids
                finished = (self.tokenizer.eos_token_id in first_ids)

                if (not finished) and ("</think>" not in first_response):
                    ...
                    early_len = len(self.tokenizer(early_stopping_text, add_special_tokens=False).input_ids)
                    remaining = self.max_new_tokens - self.thinking_budget - early_len
                    remaining = max(1, remaining)

                    second = self._server_completion(
                    ...
                    response = first_response + early_stopping_text + second

                else:
                    if ("</think>" in first_response) and (not finished):
                        continued_text = text + first_response
                        remaining = max(1, self.max_new_tokens - self.thinking_budget)
                        ...
                        response = first_response + second
                    else:
                        response = first_response.strip()

            else:
                response = self._server_completion(
                    prompt=text,
                    max_tokens=self.max_new_tokens,
                    temperature=0.7,
                    stop=None,
    ...
    def _server_completion(self, prompt: str, max_tokens: int, temperature: float = 0.7, stop=None) -> str:
